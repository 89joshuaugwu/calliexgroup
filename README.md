...
**Redesign pass (this file's current state):** the visual/interaction layer was audited against ANTI_VIBE_CODING_RULES and reworked in two passes — first the foundation (hero background, card treatment, radius scale, nav contrast, reduced-motion coverage, products catalog CMS wiring), then a full sweep applying the same radius scale and card treatment to every remaining component (`OfficesGrid`, `CoreValues`/`TeamGrid`, `Awards`, `History`, `MissionVision`, `ChinaSection`, `FeatureSlides`, `PaymentBand`, `InnovationSection`, `BoardGrid`), removing the last stray glow/shimmer/radius values, sanitizing the admin-authored rich-text field in `History.tsx` with DOMPurify before it hits `dangerouslySetInnerHTML`, and deleting `TiltCard.tsx` once nothing imported it anymore (`BoardGrid` keeps its own inline tilt — that's the one deliberate signature interaction left; every other card uses the plain `.cx-card` hover). Re-validated the same way after each pass: `npm install` clean, `tsc --noEmit` clean, `eslint` clean. `next build` reached full compilation and the TypeScript build pass with zero errors both times; it then failed at page-data collection for two *environmental* reasons only — no `FIREBASE_ADMIN_*` credentials in this sandbox, and Google Fonts blocked by egress rules — confirmed by temporarily stubbing both out and re-running: full build succeeded end to end with those two removed. Neither applies once this runs somewhere with real credentials and open network access.

Still open: Cloudinary migration off the Unsplash/blordgroup placeholders (needs real photography + your Cloudinary account, tracked in § 5 above).
