        ...
        <RevealStagger className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          {team.images.map((src, i) => (
            <motion.div
              ...
              className="group relative aspect-[16/10] overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-line)] bg-white shadow-sm"
            >
              <Image
                src={src}
                alt={`${team.title} — Callie X Group`}
                fill
            ...
            <div className="cx-card group flex h-full flex-col justify-between border border-[var(--color-line)] bg-white p-7 shadow-[var(--shadow-card)]">
              <div>
                <div className="flex items-center justify-between">
                  <span className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-badge)] bg-[var(--color-brand)] font-mono text-xs font-bold text-white">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="h-1.5 w-1.5 rounded-full bg-[var(--color-brand)]/30 transition-colors group-hover:bg-[var(--color-brand)]" />
                </div>
                <h3 className="mt-6 font-display text-lg font-bold text-[var(--color-graphite)] transition-colors group-hover:text-[var(--color-brand)]">
                  {item.title}
                </h3>
